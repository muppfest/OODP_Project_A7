INSERT INTO students (name, email, phoneNr, city, address, programCode)
VALUES ('Felix', 'felix.palmberg@gmail.com', '0734176067', 'Gävle', 'södrakungsgatan 56B', 'HIG-19104');
INSERT INTO students (name, email, phoneNr, city, address, programCode)
VALUES ('Marcus', 'marcus@gmail.com', '07072957265', 'Storvik', 'Stoviksvägen 22', 'HIG-19104');
INSERT INTO students (name, email, phoneNr, city, address, programCode)
VALUES ('Fabian', 'fabian.palmberg@gmail', '0734177072', 'Älvkarleby', 'Västanåvägen 20', 'HIG-29815');
INSERT INTO students (name, email, phoneNr, city, address, programCode)
VALUES ('Alfred', 'Alfred.nilsson@hotmail.se', '0764789022', '', '', 'HIG-19200');

INSERT INTO teachers (name, email, office)
VALUES ('Åke', 'ake@hig.se', '99530');
INSERT INTO teachers (name, email, office)
VALUES ('Carina', 'carina@hig.se', '99532');
INSERT INTO teachers (name, email, office)
VALUES ('Ann-sofie', 'annsofie@hig.se', '99531');
INSERT INTO teachers (name, email, office)
VALUES ('Anders', 'anders@hig.se', '99540');

INSERT INTO groups (name, description)
VALUES ('Labbgrupp', 'Grupp för laborationer i OODP');
INSERT INTO groups (name, description)
VALUES ('Projektgrupp', 'Grupp för projekt i Databasteknik');
INSERT INTO groups (name, description)
VALUES ('PM-grupp', 'Grupp för att samskriva ett PM i datorsystem');
INSERT INTO groups (name, description)
VALUES ('Rapportskrivning', 'Grupp för att skriva rapport i OODP');